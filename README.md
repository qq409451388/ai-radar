...
新版界面按任务而不是数据库表组织为：今日雷达、情报收件箱、资讯源、知识地图、我的进展、自动化与设置。默认以最近 90 天为当前跟进窗口，同时保留终身覆盖分数和每次覆盖评估历史。
...
启动器固定使用 `http://localhost:8501`。如果检测到该端口上有本项目上一次遗留的进程，会先停止旧进程再启动当前代码；如果端口属于其他程序，则明确报错，不会静默改到 `8502` 导致浏览器仍显示旧页面。
...
| `AI_RADAR_AI_CONCURRENCY` | 同一阶段最多并行的 AI 请求数，默认 4、上限 8 |
...
- 「资讯源」单独管理来源、连接测试和自动采集开关。
- 「自动化与设置」查看处理状态、最近 20 条任务、可读的定时计划、Token 用量与响应缓存。

### 手动流水线

点击左侧栏醒目的「运行 / 重跑更新」会打开更新中心，提供三种更新范围：

- 完整更新：采集资讯 → AI 分析 → 同步记忆 → 关联评估 → 更新快照。
- 仅更新情报：采集资讯 → AI 分析 → 关联评估 → 更新快照。
- 仅同步记忆：同步记忆 → 更新快照。

手动流水线在后台线程中运行，切换页面或关闭更新中心不会中断；弹窗会动态刷新总体进度和每个子步骤的进度。AI 分析、个人事实抽取和知识覆盖评估会在阶段内并发请求，每个工作单元使用独立数据库会话。为避免重复消耗 Token，同一时间只允许一条手动流水线运行。关闭运行 `run.sh` / `run.bat` 的终端或停止应用进程会中断任务，下次启动时会将其标记为「已中断」，可重新运行。
...
可在独立的「资讯源」页面新增、测试或启停，也可以调整 `config/default_sources.yaml` 后重启。新来源默认关闭，单点测试通过后才允许开启自动采集。
